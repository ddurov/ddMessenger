...
public class globals extends AppCompatActivity {
    ...
    public static String executeApiMethodGet(String function, String method, String[][] params) {
        String[] methodsWithoutToken = {"service.getUpdates", "user.auth"};
        params = addString(params.length, params, new String[]{"method", method});
        params = addString(params.length, params, getToken() != null && !Arrays.asList(methodsWithoutToken).contains(function+"."+method) ? new String[]{"token", getToken()} : new String[]{});

        return requestGet("https://api.eviger.ru/methods/" + function + "?" + buildQuery(params));
    }
    public static String executeApiMethodPost(String function, String method, JSONObject json) {
        try {

            String[] methodsWithoutToken = {"user.registerAccount", "email.createCode", "email.confirmCode"};

            if (!Arrays.asList(methodsWithoutToken).contains(function + "." + method)) {
        ...
        return requestPost("https://api.eviger.ru/methods/" + function, json.toString());
    }
    ...
    public static Object[] getAccount(Integer UserID) {
        ...
        try {
            ...
            JSONObject json = new JSONObject(executeApiMethodGet("users", "get", new String[][]{{"id", String.valueOf(UserID)}}));

            if (json.getJSONObject("response").getInt("online") == 1) {

                data = addObject(data.length, data, true);
                data = addObject(data.length, data, json.getJSONObject("response").getString("username"));

            } else {

                data = addObject(data.length, data, false);
                data = addObject(data.length, data, json.getJSONObject("response").getString("username"));
    ...
    public static String getHashCodeEmail(String email) {
        ...
        try {
            ...
            data = jsonRequestData.getJSONObject("response").has("error") ? "ERROR: "+jsonRequestData.getJSONObject("response").getString("error") : jsonRequestData.getJSONObject("response").getString("hash");
